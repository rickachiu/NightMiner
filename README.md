...
Works on Windows and Linux. Only partial support for MacOS with apple silicone (see below).
...
If you are on MacOS, you need to delete `ashmaize_py.so`, and rename `ashmaize_py_mac.so` to `ashmaize_py.so`.
